...
jQuery.noConflict()(function ($) {
	$('.progress .bar').each(function () {
		...
		$(this).append(
			'<span class="percentage-marker">' + percentage + '&#37;</span>'
		)
	})
	...
	$(document).ready(function () {
		...
		var options = {
			...
			success: function () {
				...
				$('.mess')
					.show()
					.html('<h5>Thanks !</h5><h5>Your message has been sent.</h5>') // Change Your message post send
				$('.mess')
					.delay(3000)
					.fadeOut(function () {
						$('#form').clearForm()
...
window.onclick = function (event) {
	if (!event.target.matches('.fa-align-right')) {
